...
const operationId = (v) => typeof v === "string" && /^[a-zA-Z0-9-]{16,80}$/.test(v);
const text = (v, limit, required = false) => {
    if (typeof v !== "string" ||
        v.trim().length > limit ||
        (required && !v.trim()) ||
        /[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(v))
        fail("invalid-argument", "Un champ est manquant, trop long ou contient des caractères invalides.");
    return v.trim();
};
const ids = (v, required = true) => {
    if (!Array.isArray(v) ||
        v.length > 500 ||
        (required && !v.length) ||
        v.some((x) => !id(x)) ||
        new Set(v).size !== v.length)
        fail("invalid-argument", "Sélection de salariés invalide (500 maximum, sans doublon).");
    return [...v].sort();
};
const hash = (v) => (0, crypto_1.createHash)("sha256").update(JSON.stringify(v)).digest("hex");
const person = (uid, value) => ({
    id: uid,
    name: `${value.firstName || ""} ${value.lastName || ""}`.trim() || "Salarié",
    role: String(value.role),
});
const eligibleProfile = (profile) => !!profile &&
    !profile.isDisabled &&
...
function draft(value) {
    ...
    const participantIds = ids(value.participantIds), voterIds = ids(value.voterIds);
    if (voterIds.some((x) => !participantIds.includes(x)))
        fail("invalid-argument", "Chaque électeur doit faire partie des participants.");
    if (!Array.isArray(value.options) ||
        value.options.length < (value.kind === "election" ? 1 : 2) ||
        value.options.length > 50)
        fail("invalid-argument", "Prévoyez au moins deux choix ou un candidat, et 50 maximum.");
    const options = value.options.map((option) => {
        ...
        if (value.kind === "election" &&
            ...
            fail("invalid-argument", "Les candidats doivent être des participants sélectionnés.");
        return {
            id: option.id,
    ...
    if (new Set(options.map((o) => o.id)).size !== options.length ||
        (value.kind === "consultation" &&
            new Set(options.map((o) => normalize(o.label))).size !==
                options.length) ||
        (value.kind === "election" &&
            new Set(options.map((o) => o.employeeId)).size !== options.length))
        fail("invalid-argument", "Les choix et candidats doivent être distincts.");
    const opensAt = text(value.opensAt, 40, true), closesAt = text(value.closesAt, 40, true);
    if (!Number.isFinite(Date.parse(opensAt)) ||
    ...
    if (!Number.isInteger(value.maxChoices) ||
        value.maxChoices < 1 ||
...
async function votingHandler(data, context, deps) {
    ...
    const audit = (tx, pollId, action, description) => tx.create(db.collection("activity_logs").doc(), {
        userId: caller.id,
    ...
    if (data?.action === "list") {
        ...
        if (data.cursor) {
            if (!id(data.cursor))
                fail("invalid-argument", "Page invalide.");
        ...
        const docs = await query.limit(31).get(), page = docs.docs.slice(0, 30);
        const polls = await Promise.all(page
            .filter((s) => mayRead(s.data()))
            .map(async (s) => visible(s.data(), s.id, (await s.ref.collection("participation").doc(caller.id).get()).data())));
        return {
            polls,
    ...
    if (!id(data?.pollId))
        fail("invalid-argument", "Scrutin invalide.");
    const ref = db.collection(COLLECTION).doc(data.pollId), tallyRef = ref.collection("private").doc("tally");
    if (data.action === "save") {
        requireManager();
        ...
        await db.runTransaction(async (tx) => {
            const previous = await tx.get(ref), old = previous.data();
            if (old?.lastMutationId === data.requestId) {
                if (old.lastMutationHash !== hash(value))
                    fail("invalid-argument", "Cette référence a déjà servi à un autre enregistrement.");
                return;
            }
            if (old && old.status !== "draft")
                fail("failed-precondition", "Les règles sont figées après publication.");
            if ((old?.revision || 0) !== data.revision)
                fail("aborted", "Ce scrutin a été modifié ailleurs. Rechargez-le avant de reprendre vos modifications.");
            const users = await tx.getAll(...value.participantIds.map((uid) => db.collection("users").doc(uid)));
            if (users.some((s) => !eligibleProfile(s.data())))
                fail("failed-precondition", "Un participant n’a plus accès au vote. Actualisez la sélection.");
            const participants = users.map((s) => person(s.id, s.data()));
            const options = value.options.map((o) => value.kind === "election"
                ? {
                    ...o,
            ...
            audit(tx, ref.id, previous.exists ? "VOTE_UPDATED" : "VOTE_CREATED", `Scrutin ${previous.exists ? "modifié" : "créé"} : ${value.title}`);
        });
    }
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (!p)
                fail("not-found", "Scrutin introuvable.");
            ...
            if (users.some((s) => !eligibleProfile(s.data())))
                fail("failed-precondition", "Actualisez les participants avant publication.");
            tx.create(tallyRef, {
                votes: 0,
                blank: 0,
                counts: Object.fromEntries(p.options.map((o) => [o.id, 0])),
            });
            tx.update(ref, {
                status: "published",
                publishedAt: now(),
        ...
        if (!operationId(data.requestId) ||
            !Array.isArray(data.choices) ||
            data.choices.some((x) => !id(x)) ||
        ...
        return db.runTransaction(async (tx) => {
            ...
            if (!p || !p.voterIds.includes(caller.id))
                fail("permission-denied", "Vous ne faites pas partie des électeurs de ce scrutin.");
            if (!eligibleProfile(fresh) ||
            ...
            if (p.status !== "published" ||
                instant < p.opensAt ||
                instant >= p.closesAt)
                fail("failed-precondition", "Le vote n’est pas ouvert. Aucun bulletin n’a été enregistré.");
            if ((!data.choices.length && !p.allowBlank) ||
                data.choices.length > p.maxChoices ||
                data.choices.some((x) => !p.options.some((o) => o.id === x)))
                fail("invalid-argument", "Choisissez uniquement les réponses autorisées pour ce scrutin.");
            if (!tally.exists)
                fail("internal", "Urne indisponible. Aucun vote enregistré.");
            const counts = { ...tally.data().counts };
            for (const choice of data.choices)
                counts[choice]++;
            const receipt = (0, crypto_1.randomUUID)();
        ...
        await db.runTransaction(async (tx) => {
            ...
            const p = snapshot.data(), t = tally.data();
            if (!p)
                fail("not-found", "Scrutin introuvable.");
            if (p.status === "closed")
                return;
            if (p.status !== "published")
                fail("failed-precondition", "Seul un scrutin publié peut être clôturé.");
            const closureReason = text(data.reason || "", 1000, now() < p.closesAt);
            if (!t || t.votes !== participation.size || t.votes > p.voterIds.length)
                fail("data-loss", "Le contrôle de cohérence de l’urne a échoué. Aucun résultat publié ; contactez le responsable technique.");
            ...
            const rows = p.options
                ...
                .sort((a, b) => b.votes - a.votes || a.label.localeCompare(b.label, "fr"));
            const sum = rows.reduce((n, row) => n + row.votes, 0);
            if (t.blank < 0 ||
                expressed < 0 ||
                rows.some((r) => !Number.isInteger(r.votes) || r.votes < 0 || r.votes > expressed) ||
                sum < expressed ||
                sum > expressed * p.maxChoices)
                fail("data-loss", "Comptage incohérent. Contactez votre responsable technique.");
            const leaders = expressed
                ? rows
                    .filter((row) => row.votes === rows[0].votes)
                    .map((row) => row.label)
                : [];
            const eligible = p.voterIds.length, quorumRequired = Math.ceil((eligible * p.quorumPercent) / 100);
            const results = {
                eligible,
                cast: t.votes,
                blank: t.blank,
            ...
            audit(tx, ref.id, "VOTE_CLOSED", `Scrutin clôturé et résultats figés : ${p.title}`);
        });
    }
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (p.status === "cancelled")
                return;
            if (p.status === "closed")
                fail("failed-precondition", "Un résultat clôturé ne peut plus être annulé.");
            tx.update(ref, {
                status: "cancelled",
            ...
            audit(tx, ref.id, "VOTE_CANCELLED", `Scrutin annulé : ${p.title} — ${reason}`);
        });
    }
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (!p || p.status !== "closed")
                fail("failed-precondition", "Clôturez le scrutin avant de finaliser le procès-verbal.");
            if (p.minutes)
                return;
            ...
            const digest = hash({
                ...
                voters: p.voters,
                options: p.options,
                documents: p.documents.filter((d) => d.kind === "attachment"),
                results: p.results,
                ...value,
                finalizedAt,
            });
            ...
            audit(tx, ref.id, "VOTE_MINUTES_FINALIZED", `Procès-verbal finalisé : ${p.title}`);
        });
    }
        ...
        if (!types.has(meta.contentType) ||
            meta.size <= 0 ||
            meta.size > 5 * 1024 * 1024)
            fail("invalid-argument", "Document PDF, Word, JPEG ou PNG de 5 Mo maximum attendu.");
        await db.runTransaction(async (tx) => {
            const snapshot = await tx.get(ref), p = snapshot.data();
            if (!p)
                fail("not-found", "Scrutin introuvable.");
            const old = (p.documents || []).find((x) => x.id === d.id);
            if (old) {
                if (old.generation !== meta.generation)
                    fail("failed-precondition", "Le fichier a changé.");
                return;
            }
            if (!((p.status === "draft" && d.kind === "attachment") ||
                (p.status === "closed" && d.kind === "signed_minutes" && p.minutes)))
                fail("failed-precondition", "Les documents du scrutin sont figés à la publication. Le PV signé peut être ajouté après finalisation.");
            if ((p.documents || []).filter((x) => x.kind === d.kind).length >=
                (d.kind === "attachment" ? 10 : 3))
                fail("resource-exhausted", "Dix pièces jointes et trois exemplaires signés maximum par scrutin.");
            tx.update(ref, {
                documents: [
                    ...(p.documents || []),
                    {
            ...
            audit(tx, ref.id, "VOTE_DOCUMENT_ADDED", `Document ajouté au scrutin : ${p.title}`);
        });
    }
        ...
        await db.runTransaction(async (tx) => {
            ...
            audit(tx, ref.id, "VOTE_DOCUMENT_REMOVED", `Document retiré du brouillon : ${p.title}`);
        });
    }
    else if (!["get", "download_document"].includes(data.action))
        fail("invalid-argument", "Action inconnue.");
    const snapshot = await ref.get(), p = snapshot.data();
    if (!p || !mayRead(p))
