...
exports.dispatchMissionsTransaction = dispatchMissionsTransaction;
const crypto_1 = require("crypto");
const v1_1 = require("firebase-functions/v1");
const deliveryAddress_1 = require("./deliveryAddress");
const bad = (message) => { throw new v1_1.https.HttpsError('invalid-argument', message); };
const unavailable = (message) => { throw new v1_1.https.HttpsError('failed-precondition', message); };
...
const nonnegative = (value, fallback = 0) => value === undefined ? fallback : typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : bad('Durée ou distance invalide.');
...
async function dispatchMissionsTransaction(db, data, caller) {
    ...
    const missions = data.missions.map((m) => {
        ...
        if (!Number.isFinite(date.getTime()) || date.toISOString().slice(0, 10) !== m.date)
            bad('Date de tournée invalide.');
        if (drivers.has(m.driverId))
            bad('Un chauffeur apparaît sur plusieurs tournées du même dispatch.');
        drivers.add(m.driverId);
        if (m.vehicleId) {
            if (!id(m.vehicleId) || vehicles.has(m.vehicleId))
                bad('Véhicule invalide ou utilisé deux fois.');
            vehicles.add(m.vehicleId);
        }
        if (!Array.isArray(m.stops) || !m.stops.length || m.stops.length > 450)
            bad('Liste des arrêts invalide.');
        const stopIds = new Set();
        const stops = m.stops.map((s, index) => {
            ...
            for (const parcelId of s.packageIds) {
                if (!id(parcelId) || parcelIds.has(parcelId))
                    bad('Un colis ne peut figurer que dans un seul arrêt.');
            ...
            if (!stop.address || !stop.city || !stop.postalCode)
                bad('Adresse de livraison incomplète.');
            if (s.coordinates) {
                const { lat, lng } = s.coordinates;
                if (!Number.isFinite(lat) || !Number.isFinite(lng) || Math.abs(lat) > 90 || Math.abs(lng) > 180)
                    bad('Coordonnées invalides.');
                stop.coordinates = { lat, lng };
            }
            for (const key of ['timeWindowStart', 'timeWindowEnd']) {
                if (s[key] !== undefined && s[key] !== '') {
                    if (!time(s[key]))
                        bad('Créneau de livraison invalide.');
                    stop[key] = s[key];
                }
            }
            if (stop.timeWindowStart && stop.timeWindowEnd && stop.timeWindowStart >= stop.timeWindowEnd)
                bad('Créneau de livraison inversé.');
            for (const key of ['estimatedArrival', 'estimatedDeparture']) {
                if (s[key] !== undefined) {
                    if (typeof s[key] !== 'string' || !Number.isFinite(Date.parse(s[key])))
                        bad('Horaire estimé invalide.');
                    stop[key] = s[key];
                }
            }
            for (const key of ['distanceFromPrevious', 'durationFromPrevious', 'waitingTime', 'delayFromPrevious', 'breakFromPrevious'])
                if (s[key] !== undefined)
                    stop[key] = nonnegative(s[key]);
            if (typeof s.floor === 'number' && Number.isFinite(s.floor))
                stop.floor = s.floor;
            if (typeof s.hasElevator === 'boolean')
                stop.hasElevator = s.hasElevator;
            if (typeof s.notes === 'string')
                stop.notes = text(s.notes, 2000);
            return stop;
        });
        return {
    ...
    const fingerprint = (0, crypto_1.createHash)('sha256').update(stable(missions)).digest('hex');
    const receiptId = (0, crypto_1.createHash)('sha256').update(caller.id + ':' + data.requestId).digest('hex');
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (receipt.exists) {
            ...
            if (result.fingerprint !== fingerprint)
                bad('Ce dispatch a déjà été utilisé avec un autre contenu. Relancez l’optimisation.');
            return { missionIds: result.missionIds, packageCount: result.packageCount, replayed: true };
        }
        ...
        const hubIds = [...new Set(missions.map((m) => m.hubId))];
        ...
        const missionIds = missions.map((_m, index) => 'dispatch-' + receiptId + '-' + index);
        ...
        missions.forEach((m, index) => {
            ...
            if (!hub?.isActive)
                unavailable('Le hub de départ est indisponible.');
            ...
            for (const stop of m.stops) {
                for (const parcelId of stop.packageIds) {
                    ...
                    if ((0, deliveryAddress_1.placeKey)(pkg) !== (0, deliveryAddress_1.placeKey)(stop))
                        unavailable(`L’adresse du colis ${parcelId} a changé. Relancez l’optimisation.`);
            ...
            const totalPackages = m.stops.reduce((sum, stop) => sum + stop.packageIds.length, 0);
...
//# sourceMappingURL=dispatchMissions.js.map