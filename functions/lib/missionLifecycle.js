...
exports.finishMissionHandler = finishMissionHandler;
...
async function finishMissionHandler(data, context, { db, requireActiveCaller, isAdminCaller }) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (remaining.length) {
            throw new functions.https.HttpsError('failed-precondition', `${remaining.length} arrêt(s) restent à traiter. Livrez, déclarez les échecs ou faites réaffecter les colis avant de clôturer.`);
        }
        if (stops.some(s => s.proofSyncPending === true)) {
            throw new functions.https.HttpsError('failed-precondition', 'Des preuves sont encore en attente d’envoi. Synchronisez les appareils utilisés avant de clôturer.');
        }
        const ids = [...new Set(stops.flatMap(s => s.packageIds || []))];
        ...
        const proofs = await Promise.all(proofStops.map(s => tx.get(db.collection(s.type === 'PICKUP' ? 'pickups' : 'proofs_of_delivery').doc(`${id}_${s.id}`))));
        const proofByStop = new Map(proofStops.map((s, i) => [s.id, proofs[i]]));
        for (const stop of stops) {
            const packageIds = stop.packageIds || [];
            ...
            if (stop.type === 'DELIVERY') {
                if (!proof || proof.missionId !== id || proof.stopId !== stop.id || proof.driverId !== mission.driverId ||
                    ...
                    throw new functions.https.HttpsError('failed-precondition', 'Une preuve de livraison ou d’échec est absente ou incomplète. Synchronisez les preuves ; si le problème persiste, contactez le bureau.');
                }
                ...
                if (proof.type === 'SUCCESS' && (typeof proof.signatureUrl !== 'string' || !proof.signatureUrl.startsWith('https://') ||
                    ...
                    throw new functions.https.HttpsError('failed-precondition', 'Une preuve de remise est incomplète (réceptionnaire, signature ou position). Contactez le bureau.');
                }
            }
            ...
            for (const pid of packageIds) {
                ...
                if (!parcel?.exists)
                    throw new functions.https.HttpsError('failed-precondition', 'Un colis de la tournée est introuvable.');
                const p = parcel.data();
